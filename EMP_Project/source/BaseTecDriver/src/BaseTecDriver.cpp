...
namespace BaseTecDriver {
    ...
    pru(0)
  {
    ...
    try {
      ...
      anaIn = new AnalogInputs(pru.getSharedMemory() + 0x00f0, mGammaPathBase + ".AnalogInputs", mLoggerName + ".AnalogInputs", mCycleTime);
  ...
  BaseTecDriver::prepareStart()
  {
    ...
    do {
      CLOG(DEBUG, mLoggerName.c_str())<< "wait until PRU program is running. (Status: 0x" << std::hex << std::setw(8) << std::setfill('0') << ptRawData->firmwareStatus.word << "; active resources: 0x" << std::setw(4) << ptRawData->activeResources.word << ")";
      usleep(1000 * 1000);
    }while (!ptRawData->firmwareStatus.bit._bit08);

    if (ptRawData->activeResources.word != RequestedResources) {
      CLOG(WARNING, mLoggerName.c_str()) << "not all requested modules are loaded !!";
      char buf[512];
    ...
    CLOG(INFO, mLoggerName.c_str())<< "PRU program successful started (active modules: 0x" << std::hex << std::setw(4) << std::setfill('0') << ptRawData->activeResources.word << ")";
  }
  ...
  BaseTecDriver::cleanUp()
  {
    ...
    do {
      CLOG(INFO, mLoggerName.c_str()) << "wait until PRU program is stopped. (Status: 0x" << std::hex << std::setw(8) << std::setfill('0') << ptRawData->firmwareStatus.word << ")";
      usleep(1000 * 1000);
  ...
  BaseTecDriver::execCommand(std::string& _cmdLine)
  {
    ...
    // -------------------------------------------------------------------------------------------------------
    // unknown command: call function of base class
    // -------------------------------------------------------------------------------------------------------
    else {
      GaAppBaseLib::ModuleInterface::execCommand(_cmdLine);
    }
  }
