...
namespace KleberMqtt
{
              ...
              taskCounter(0)
    {
        ...
        for (ioIndex = 0; ioIndex < outputCount; ioIndex++)
        {
            ...
            digOutErrorStatus.connectToPv("HardwareResources.BaseTecDriver.DigitalOutputs.Output_07.reqState", ga_PV_PROPERTY_ACC_NON_BLOCKING);
        }
    ...
    int32_t KleberMqtt::taskRun(gs_Task taskHandle, const uint32_t* frameNumber) noexcept
    {
        ...
        // zum Debuggen der NutzenMagazinZuordnung
        // wenn dort ein Fehler auftrat, wird die Hupe aktiviert
    	if (digOutErrorStatus.get())
    	{
    		buzzerConfirmed = false;
    		digOutErrorStatus.put(0); // gesetzt in AsymtecUnloader.cpp
    	}
        // Ende Debug NutzenMagazinZuordnung
        ...
        if (outSignalTower_Buzzer > 0)
        {
            if (!buzzerConfirmed)
            {
            	...
            	digOutSignalRed.put((taskCounter >= 5) ? 1 : 0);
            }
            else
            {
                digOutSignalBuzzer.put(0);
            	digOutSignalRed.put(0);
            }
        }
    ...
    void KleberMqtt::mqttConnlost(void *context, char *cause)
    {
		...
		{
    		CLOG(INFO, mLoggerName.c_str())<< "Call of Connection Lost but still connected";
		}
		else
		{
			...
			if (cause == nullptr)
			{
				CLOG(INFO, mLoggerName.c_str())<< "MQTTClient_disconnected = " << "no cause available";
			}
			else
			{
				CLOG(INFO, mLoggerName.c_str()) << "MQTTClient_disconnected = " << cause;
			}
